...
if command -v lsof >/dev/null 2>&1; then
  ...
  if [[ -n "${PIDS}" ]]; then
    for pid in $PIDS; do
      ...
      # Allow reuse when the listener is already this repo's Vite.
      if [[ -n "${cwd:-}" && ("$cwd" == "$ROOT" || "$cwd" == "$ROOT"/*) ]]; then
        echo "Vite already listening on :$PORT from $cwd — reusing"
      ...
      echo "stop that process (or remove a sibling 'deckhand copy' tree), then retry." >&2
      exit 1
    done
  fi
fi
